...
In this version, we code inter-trial fixation (4s, 2TRs) following each sentence as cond 0, i.e. the only fixation is
12s in the beginning, middle and end of each run.
...
def main(raw_args=None):
    ...
    parser.add_argument('--UID', default='875', type=str, help='UID str')
    ...
    if args.external_output_root is None:
        output_root = join(root, 'output_glmsingle_no_fix')
    else:
        output_root = join(args.external_output_root, 'output_glmsingle_no_fix')
